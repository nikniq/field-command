...
class MenuScene:
    ...
    def _draw_content(self, screen, mouse):
        ...
        from .defs import MODE_BY_ID
        mode = MODE_BY_ID.get(settings.mode, MODE_BY_ID["annihilation"])
        picks = [(300, f"Map: {meta['name']}  ({meta['players']}p)", settings.cycle_map),
                 (150, f"Opponents: {n}", settings.cycle_opponents),
                 (170, "Teams: Free-for-all" if tc < 2 else f"Teams: {tc}", settings.cycle_teams),
                 (210, f"Mode: {mode[1]}", settings.cycle_mode)]
