...
from .defs import (GAS, AMBER, BAD, BUILDINGS, COLOR_NAMES, CRYSTAL, DIM, GOOD, KITS, KOTH_HOLD, TEAM_COLOR, TEAM_LIGHT, TEXT,
...
class HUD:
    ...
    def _draw_top(self, screen, mouse):
        ...
        ai_ = art.sprites.get("aicon18", art.gas_icon(), 0, 18 / 40)
        screen.blit(ai_, (118 - ai_.get_width() / 2, cy - ai_.get_height() / 2))
        ui.blit_text(screen, str(int(getattr(g.s, "gas", 0))), 17, GAS, (132, cy), bold=True, mono=True)
    ...
    def _draw_card(self, screen, mouse):
        ...
        for i in range(10):
            ...
            if b.cost is not None:
                ...
                if b.gas:
                    ok_a = getattr(g.s, "gas", 0) >= b.gas
                    ui.blit_text(screen, str(b.gas), 10, GAS if ok_a else BAD, (r.right - 5, r.y + 22), align="right", mono=True)
        ...
        for i, r in enumerate(self.button_rects):
            if r.collidepoint(mouse) and i < len(self.current_buttons):
                ...
                if b.cost is not None:
                    ...
                    if b.gas:
                        r2 = ui.blit_text(screen, str(b.gas), 13, GAS, (r.x - 10 - ci.get_width(), y + 18), align="right", mono=True)
                        ai_ = art.sprites.get("aicon14", art.gas_icon(), 0, 14 / 40)
