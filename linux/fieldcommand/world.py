...
class World:
    def __init__(self, map_spec, players, difficulty, mission=None, seed=None, mode="annihilation",
        ...
        self.gas = {s: float(GAS_START) for s in self.players}
    ...
    def modify_damage(self, victim, amount, attacker):
        ...
        if not victim.is_building and victim.dug_in:
            amount *= ENTRENCH_FACTOR
    ...
    def _build(self, slot, worker_id, kind, x, y, queue):
        ...
        if s.requires and not self.has_built(s.requires, slot):
            ...
            self.emit("msg", slot, "Not enough crystal", "bad")
        elif self.spend_gas(slot, GAS_BUILD.get(kind, 0)):
    ...
    def _upgrade(self, slot, bs, kind):
        ...
        for b in bs:
            ...
            if self.resources[slot] < cost:
                self.emit("msg", slot, "Not enough crystal", "bad")
                return
            if not self.spend_gas(slot, GAS_UPGRADE.get(kind, 0)):
                return
            self.resources[slot] -= cost
    ...
    def has_tech(self, kind, team):
        """Tech is side-wide: any standing building of the side that researched it counts."""
        return any(b.team == team and not b.dead and kind in b.upgrades for b in self.buildings)
    ...
    def refund(self, amount, team, gas=0):
        self.resources[team] += amount
        if gas:
            self.gas[team] += gas

    def spend_gas(self, slot, amount):
        """Takes `amount` gas from the side if it has it; says so if it does not."""
        if amount <= 0:
            return True
        if self.gas[slot] < amount:
            self.emit("msg", slot, "Not enough gas — build a Refinery", "bad")
            return False
        self.gas[slot] -= amount
        return True
    ...
    def train(self, kind, bs, team):
        ...
        if self.supply_used(team) + s.supply > self.supply_cap(team):
            ...
            return False
        if not self.spend_gas(team, GAS_COST.get(kind, 0)):
            return False
    ...
    def unbuild(self, b):
        ...
        if b.built or b.dead or b.progress >= UNDO_PROGRESS:
            return False
        self.refund(b.stats.cost, b.team, GAS_BUILD.get(b.kind, 0))
        b.dead = True
    ...
    def cancel_queue(self, b, index):
        if 0 <= index < len(b.queue):
            ...
            self.refund(UNITS[k].cost, b.team, GAS_COST.get(k, 0))
    ...
    def deposit(self, amount, team, x, y):
        ...
        self.emit("income", team, x, y, amount, 0)
