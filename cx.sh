...
target_test() {
    ...
    if on_macos; then
        ...
        if [ -x "$bin" ]; then
            for t in FC_WORLDTEST FC_REPAIRTEST FC_TEAMSTEST FC_SIEGETEST FC_UPGRADETEST FC_TOWERTEST FC_STORETEST FC_AITEST FC_AUDIOTEST FC_MEDICTEST FC_PINGTEST; do
                step "macOS: $t"
